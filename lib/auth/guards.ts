...
/** Clipper or network with vetted marketplace access — the (app) shell rule. */
export async function requireActiveClipper(): Promise<Profile> {
  const user = await requireRole("clipper", "network");
 ...
 * (admin, agency) or an APPROVED clipper/network (access "active"). Everyone else
...
export function routeFor(user: Profile): string {
  ...
  if (user.role === "agency") return user.profileCompleted ? "/agency" : "/onboarding";
  // clipper / network — active only (anyone else is refused at sign-in)
